...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/me').then((r) => r.json()).then((d) => setAdminEmail(d.email || '')).catch(() => undefined);
  }, []);
  ...
  const exportCsv = () => {
    ...
    const rows = items.map((s) => [s.createdAt, s.formType, s.fullName, s.email, s.company ?? '', s.serviceInterest ?? '', String(s.consent_checked), s.status]);
  ...
  const updateStatus = async (id: string, next: Submission['status']) => {
    await fetch(`/api/admin/submissions/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: next }) });
    load();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-2xl font-bold">Admin Dashboard <span className="text-green-400 text-sm">● Live</span></h1>
